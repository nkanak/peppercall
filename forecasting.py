...
class SingleShotForecaster:
    ...
    def __init__(self, number_of_lstm_units:int, prediction_length:int) -> None:
        ...
        tf.keras.layers.LSTM(self.__number_of_lstm_units, return_sequences=False),
        # Shape => [batch, out_steps*features].
        tf.keras.layers.Dense(self.__prediction_length,
    ...
    def fit(self, window: WindowGenerator, epochs: int, epochs_to_stop:int=5) -> tf.keras.callbacks.History:
        ...
        early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss',
                                                    patience=epochs_to_stop,
                                                    mode='min')
        history = self.__model.fit(window.train, epochs=epochs,
