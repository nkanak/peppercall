...
def resample_dataset(
    df: pd.DataFrame, aggregation=Literal["QS", "MS", "W", "D"]
) -> pd.DataFrame:
...
def split_dataset(
    df: pd.DataFrame, train_percentage: float, validation_percentage: float
    ...
    val_df = df[
        int(n * train_percentage) : int(n * (train_percentage + validation_percentage))
    ]
...
class WindowGenerator:
    ...
    def __init__(
        ...
        if label_columns is not None:
            self.__label_columns_indices = {
                name: i for i, name in enumerate(label_columns)
            }
        self.__column_indices = {name: i for i, name in enumerate(train_df.columns)}
    ...
    def split_window(self, features):
        ...
        if self.__label_columns is not None:
            labels = tf.stack(
                [
                    labels[:, :, self.__column_indices[name]]
                    for name in self.__label_columns
                ],
                axis=-1,
            )
